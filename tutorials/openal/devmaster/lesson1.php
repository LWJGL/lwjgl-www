<div class="paragraph_item">
    <h2>Single Static Source: Lesson 1</h2>
...
the sound.</p>
<pre class="code"><span class="codeComment">  /** Position of the source sound. */</span>
...
<p>In the above code we specify the position and velocity of the source and 
listener objects. These NIO Buffers are vector based Cartesian coordinates. 
<br><i><b>Note:</b> LWJGL uses the position, limit and capacity
properties of NIO buffers to determine where to index the data and how many elements to
get/set. It is therefore crucial that these limits are set correctly. In the above case
we would have to flip each of the buffers so their position and limit is set correctly. A newly created
buffer will have its position set to 0 and its limit to its capacity.</i></p>
...
<br><i><b>Note:</b> The original tutorial uses ALUT to load wave data. ALUT is not available
in the LWJGL binding, due to license issues. You may use the <code>WaveData</code> class to load
sound files instead.</i></p></p>
<pre class="code"><span class="codeComment">    // Load wav data into a buffer.</span>
...
bit.</p>
<p>The WaveData class is very helpful here. It opens up the file for us and gives 
us all the information we need to create the buffer. And after we have attached 
all this data to the buffer it will help use dispose of the data. It all works 
in a clean and efficient manner.</p>
<pre class="code">    <span class="codeComment">// Bind the buffer with the source.</span>
...
have done above).</p>
<pre class="code">    <span class="codeComment">// Do another error check and return.</span>
    <span class="codeKeyword">if</span> (AL10.alGetError() == AL10.AL_NO_ERROR)
...
<p>This is our main entry point. We just create our <code>Lesson1</code> instance and call the
execute method</p>
<pre class="code">
...
<p>The function 'AL.create()' will setup everything that OpenAL needs to do for 
us. Basically 'AL.create()' creates a single OpenAL context through Alc and sets it to 
current. On the Windows platform it initializes DirectSound. We also do an 
initial call to the error function to clear it. Every time we call 'alGetError' 
it will reset itself to 'AL_NO_ERROR'.</p>
<pre class="code"><span class="codeComment">    // Load the wav data.</span>
...
the program. Then we update the listener values.</p>
...
sample. Pressing 'q' will exit the program. When done, we kill all data loaded into buffers and delete
any source objects created.</p>
