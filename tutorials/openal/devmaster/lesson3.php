<div class="paragraph_item">
    <h2>Multiple Sources: Lesson 3</h2>
...
<p>Hello. It's been a while since my last tutorial. But better late than never I guess. 
Since I'm sure your all impatient to read the latest tutorial, I'll just jump right 
into it. What we hope to accomplish with this one is to be able to play more that one 
audio sample at a time. Very intense games have all kinds of stuff going on usually 
involving different sound clips. It won't be hard to implement any of this though. Doing 
multiple sounds is similar to doing just one.</p>
<pre class="code">
...
<p>I guess this little piece of source code will be familiar to a lot of you who've read the first 
two tutorials. The only difference is that we now have 3 different sound effects that we are going to 
load into the OpenAL sound system.</p>

<pre class="codeFixed"><span class="codeComment">  /**
   * boolean LoadALData()
   *
...
<p>This code looks quite a bit different at first, but it isn't really. Basically 
we load the file data into our 3 buffers, then lock the 3 buffers to our 3 sources 
relatively. The only other difference is that the "Battle.wav" (Source index 0) is looping 
while the rest are not.<br>
<i><b>NOTE:</b> Notice how the buffers position is changed. Since we know a vector in the buffer
consists of 3 floats, we use this knowledge to index into the buffer and position us precisely where
the next vector begins. We could have used multiple buffers, but this is quicker, and takes up less code
- at a slight cost of added complexity.</i></p>
...
<pre class="codeFixed">  <span class="codeKeyword">public static void</span> main(String[] args) {
...
<p>Here is the interesting part of this tutorial. We go through each of the sources to make sure 
it's playing. If it is not then we set it to play but we select a new point in 3D space 
for it to play (just for kicks).
And bang! We are done. As most of you have probably seen, you don't have to do anything 
special to play more than one source at a time. OpenAL will handle all the mixing 
features to get the sounds right for their respective distances and velocities. And 
when it comes right down to it, isn't that the beauty of OpenAL?
You know that was a lot easier than I thought. I don't know why I waited so long to 
write it. Anyway, if anyone reading wants to see something specific in future 
tutorials (not necessarily pertaining to OpenAL, I have quite an extensive knowledge base) 
drop me a line at <a href="mailto:lightonthewater@hotmail.com">lightonthewater@hotmail.com</a>
I plan to do tutorials on sharing buffers and the Doppler effect 
in some later tutorial unless there is request for something else. 
Have fun with the code!</p>
<p>
Download source code and resources for this lesson <a href="tutorials/openal/devmaster/lesson3.zip">here</a>
</p> 
</div>