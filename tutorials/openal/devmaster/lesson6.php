<div class="paragraph_item">
    <h2>Advanced Loading and Error Handles: Lesson 6</h2>
...
<p>
We will first consider a few functions that will help us out a lot by the time we have finished.</p>
...
as a limited resource, and will handle that resource efficiently.</p>
...
<p>This function will convert an OpenAL error code to a string so it 
...
errors so that our code will be up to date with later versions.</p>
<pre class="code"><span class="codeComment">  /**
   * 1) Identify the error code.
   * 2) Return the error as a string.
   */  </span>
...
<p>This function will perform a similar task as the previous one accept this one will 
interpret Alc errors. OpenAL and Alc share common id's, but not common enough 
and not dissimilar enough to use the same function for both.</p>
<p>One more note about the function 'alGetError': The OpenAL sdk defines that it 
...
in the error bit if no previous error is already stored there.</p>
<pre class="code">
  <span class="codeComment">/**
   * 1) Creates a buffer.
   * 2) Loads a wav file into the buffer.
...
<p>As you can see we do an error check at every possible phase of the load. 
...
can be passed to any one of the OpenAL functions which will generate an error.</p>
<pre class="code">
  <span class="codeComment">/**
   * 1) Checks if file has already been loaded.
   * 2) If it has been loaded already, return the buffer id.
...
<p>This will probably be the piece of code most people have trouble with,
...
<p>Now that we have created a system which will handle the buffers for us, we just need an 
...
to the new source. We save the source id internally and also return it.</p>
<pre class="code">
  <span class="codeComment">/**
   * 1) Releases temporary loading phase data.
   */</span>
...
<p>The global arraylist 'loadedFiles' stored the file path of every wav file that was loaded 
into a buffer. It doesn't make sense to keep this data lying around after we have loaded all 
of our data, so we will dispose of it.</p>
<pre class="code">
...
<p>We have seen the function in previous tutorials. It will represent the part of a program which 
...
playback. There is no more concern for handling buffers because the system will handle them automatically.</p>
<pre class="code">
<span class="codeComment">  /**
   * 1) Releases all buffers.
   * 2) Releases all sources.
...
<p>All along we have been storing the buffer and source id's into array lists. We free all the buffers 
and sources by going through them and releasing them individually. After which we destroy 
the lists themselves. All we need to do now is catch the OpenAL errors that we have thrown.</p>
<pre class="code">
...
<p>If something has gone wrong during the course of the load we will be notified of it right away. 
When we catch the error it will be reported on the console. We can use this for debugging or 
general error reporting.</p>
<p>
...
tutorials. Expect future tutorials to expand on this code.</p>
</div>